...
////////////Algorithm Kosaraju\\\\\\\\\\
    ...
    private void DFS2(int v, bool[] visited, List<int> component, List<List<int>> graph)
    {
        visited[v] = true;
        component.Add(v);
    ...
    public List<List<int>> FindSCC()
    {
        ...
        for (int i = 0; i < n; i++)
            if (!visited[i])
                DFSKosaraju(i, visited, st);

        List<List<int>> transpose = TransposeGraph();

        Array.Fill(visited, false);
        List<List<int>> scc = new();
...
//////////////////////////Algorithm Tarjan/////////////////////////////
    ...
    public List<List<int>> TarjanSCC()
    {
        int n = adjacencyList.Count;
        int[] disc = new int[n];
        int[] lowLinkValue = new int[n];
        Stack<int> stack = new();
        bool[] inStack = new bool[n];
        List<List<int>> sccTarjan = new();
        ...
        {
            if(disc[i] == -1)
                TarjanDFS(i, ref index, disc, lowLinkValue, inStack, stack, sccTarjan);
        }

        return sccTarjan;
    }    
    
    private void TarjanDFS(int u, ref int index, int[] disc, int[] lowLinkValue, bool[] inStack, Stack<int> stack, List<List<int>> sccTarjan)
    {
        disc[u] = lowLinkValue[u] = index++;
        stack.Push(u);
        inStack[u] = true;
        ...
        foreach(int v in adjacencyList[u])
        {
            if(disc[v] == -1)
            {
                TarjanDFS(v, ref index, disc, lowLinkValue, inStack, stack, sccTarjan);
                lowLinkValue[u] = Math.Min(lowLinkValue[u], lowLinkValue[v]);
            }
            else if(inStack[v])
            {
                lowLinkValue[u] = Math.Min(lowLinkValue[u], disc[v]);
            }
        }

        if(lowLinkValue[u] == disc[u])
        {
            List<int> component = new();
